...
def guess_constraint(series: pd.Series, col_name: str) -> tuple[str, str]:
    """
    ...
    Returns (constraint_type_guess, reasoning_note).
    """
    ...
    # Sentinel-aware note (matches project's HELOC finding — flag, don't auto-classify)
    if min_val < -1:
...
def generate_scaffold(processed_dir: Path, out_path: Path):
    ...
    for f in files:
        ...
        for col in df.columns:
            guess, note = guess_constraint(df[col], col)
            rows.append({
...
def compute_invalid_value(train_series: pd.Series, constraint_type: str, rng: np.random.Generator) -> float:
    """
    Per D1 §8: v_invalid chosen per-feature from the domain constraint annotation.
      ...
      range       -> beyond the documented range bound
      none        -> beyond train-fold IQR-based bound
    All statistics computed from TRAIN FOLD ONLY per D2 Decision 4.
    """
    ...
    if constraint_type == "no_negative":
        ...
        # Without an explicit documented bound in this scaffold, fall back to
        # IQR-based offset beyond the train-fold max — flagged as a D4 follow-up
        # if per-column explicit bounds are added to the annotation later.
    ...
    for _, row in ds_annotation.iterrows():
        ...
        if n_corrupt == 0:
            metadata["per_column"][col] = {
                "constraint_type": constraint_type, "v_invalid": v_invalid,
                "target_rate": alpha, "achieved_rate": 0.0, "n_corrupted": 0,
            }
            continue

        selected_idx = rng.choice(df.index, size=n_corrupt, replace=False)
        df.loc[selected_idx, col] = v_invalid

        metadata["per_column"][col] = {
            "constraint_type": constraint_type,
            "v_invalid": float(v_invalid),
            "target_rate": alpha,
...
def validate_injection(original_df: pd.DataFrame, corrupted_df: pd.DataFrame, metadata: dict) -> dict:
    ...
    for col, meta in metadata["per_column"].items():
        ...
        changed_mask = original_df[col] != corrupted_df[col]
        n_changed = changed_mask.sum()
        mechanism_ok = n_changed == meta["n_corrupted"]
        # Mechanism fidelity: every changed cell must equal v_invalid by construction
        if n_changed > 0:
...
def main():
    ...
    if args.mode == "scaffold":
        ...
        for col, meta in metadata["per_column"].items():
            ...
            print(f"  {col:<30} type={meta['constraint_type']:<12} "
                  f"v_invalid={meta['v_invalid']:.3f}  "
