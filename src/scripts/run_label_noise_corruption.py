...
Full-scale runner for D3's label_noise.py module (Uniform, Class-Conditional)
against the real processed corpus.

CHANGE FROM SMOKETEST VERSION (D7 pre-flight review): conditioning_feature
and validation_details_raw added to the manifest row. label_noise has no
conditioning structure (conditioning_feature will be None here), but the
field is included for schema consistency across all six manifests.

Structural differences from the missingness/feature_noise runners:
  ...
  - Class-conditional needs Cleanlab to estimate the asymmetry ratio (D1 Sec
    ...
    whole run — the uniform-noise rows still complete.

Usage (full-scale):
    python -m src.scripts.run_label_noise_corruption --all --seeds 1 2 3 4 5
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run D3's label_noise module against real processed data (full scale).")
    ...
    log(f"LABEL NOISE FULL RUN — {len(selected)} dataset(s), "
